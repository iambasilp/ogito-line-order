...
import { LogOut, Users, ShoppingCart, Menu, X, MapPin, BarChart2, Target, Camera, User as UserIcon, Info, Gamepad2 } from 'lucide-react';
        ...
        <div className="flex-1 overflow-y-auto py-4 px-3 space-y-1">
          ...
          <Link to="/game" onClick={() => setMobileMenuOpen(false)}>
            <Button
              ...
              size="sm"
              className={`w-full justify-start font-medium mb-1 ${
                isActive('/game') ? 'bg-white/20 text-white' : 'text-white/80 hover:text-white hover:bg-white/10'
              }`}
            >
          ...
          <div className="sm:hidden mt-2">
            {user && (
              <GlobalChatIcon className="w-full justify-start font-medium mb-1" showLabel />
            )}
          </div>
        </div>
