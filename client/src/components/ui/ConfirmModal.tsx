      ...
      <DialogContent className="w-[95vw] sm:max-w-md p-6">
        <DialogHeader className="border-b-0 pb-0">
          <div className="flex items-center gap-3">
            ...
            <DialogTitle className="text-xl text-gray-900 border-none">{title}</DialogTitle>
          </div>
          <DialogClose onClose={onClose} />
        </DialogHeader>
        
        <div className="pt-2 pb-6 px-1 text-sm text-gray-600">
          {description}
        </div>
        ...
        <div className="flex flex-col-reverse sm:flex-row justify-end gap-2 pt-4 border-t border-gray-100">
          ...
          <Button
            type="button"
            className={`w-full sm:w-auto ${variant === 'danger' ? 'bg-red-600 hover:bg-red-700 text-white shadow-sm shadow-red-200' : ''}`}
            onClick={() => {
              onConfirm();
              onClose();
            }}
