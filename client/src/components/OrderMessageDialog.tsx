    ...
    return (
        <Dialog open={open} onOpenChange={onOpenChange}>
            ...
            ">
                ...
                >
                    {sortedMessages.length === 0 ? (
                        <div className="flex flex-col items-center justify-center h-full text-center text-muted-foreground">
                            <div className="bg-white/80 backdrop-blur-sm p-4 rounded-full mb-4 shadow-sm border border-white/50">
                                <MessageSquare className="h-12 w-12 opacity-20 text-emerald-600" />
                            </div>
                            <p className="text-sm font-bold text-gray-700 bg-white/60 backdrop-blur-sm px-4 py-1 rounded-full border border-white/40 shadow-sm">
                                ...
                                <div key={idx} className={`flex flex-col gap-1 max-w-[85%] ${msg.role === 'admin' ? 'ml-auto items-end' : 'mr-auto items-start'}`}>
                                    ...
                                    <div
                                        className={`relative p-2.5 text-sm shadow-[0_1px_0.5px_rgba(0,0,0,0.13)] max-w-full ${
                                            msg.role === 'admin'
                                            ? 'bg-[#dcf8c6] border-none rounded-lg rounded-tr-none text-gray-800 self-end'
                                            : 'bg-white border-none rounded-lg rounded-tl-none text-gray-800 self-start'
                                        }`}
                                    >
                                        {/* Bubble Tail */}
                                        <div className={`absolute top-0 w-3 h-3 ${
                                            msg.role === 'admin' 
                                            ? '-right-2.5 border-l-[10px] border-l-[#dcf8c6] border-b-[10px] border-b-transparent' 
                                            : '-left-2.5 border-r-[10px] border-r-white border-b-[10px] border-b-transparent'
                                        }`} />
                                        ...
                                        {isEditing ? (
                                            <div className="flex flex-col gap-2 min-w-[200px]">
                                                <textarea
                                                    ...
                                                    className="w-full p-2 text-sm border-none bg-black/5 rounded resize-none focus:outline-none"
                                                    rows={3}
                                                    maxLength={1000}
                                                    autoFocus
                                                />
                                        ...
                                        <div className="flex items-center justify-end gap-1.5 -mt-0.5 ml-8 h-4 overflow-hidden">
                                            ...
                                            {msg.role === 'admin' && (
                                                <div className="flex items-center">
                                                    <CheckCircle className={`h-3 w-3 ${msg.status === 'approved' ? 'text-blue-500 fill-blue-500/20' : 'text-gray-400'}`} />
                                                </div>
                                            )}
                                        </div>
                                    </div>
                ...
                {/* Footer Input */}
                <div className="p-2 pb-4 bg-[#f0f2f5] border-t mt-auto">
                    <form onSubmit={handleSubmit} className="flex w-full gap-2 items-end max-w-full">
                        <div className="flex-1 flex items-end bg-white rounded-2xl shadow-sm border border-gray-200/50 min-h-[44px] px-2 py-1 gap-1">
                            <Popover open={showEmojiPicker} onOpenChange={setShowEmojiPicker}>
                                <PopoverTrigger asChild>
                                    <Button
                                        ...
                                        variant="ghost"
                                        size="icon"
                                        className="h-9 w-9 rounded-full text-gray-500 hover:text-emerald-600 hover:bg-emerald-50 shrink-0"
                                    >
